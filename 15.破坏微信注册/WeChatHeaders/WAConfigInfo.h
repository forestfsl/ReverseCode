//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "WCTTableCoding-Protocol.h"

@class NSData, NSString;

@interface WAConfigInfo : NSObject <WCTTableCoding>
{
    unsigned int type;
    unsigned int version;
    unsigned int updateTime;
    NSString *appID;
    NSData *configData;
}

+ (void)__wcdb_WAConfigInfo_multi_primary_6:(struct WCTBinding *)arg1;
+ (void)__wcdb_WAConfigInfo_multi_primary_5:(struct WCTBinding *)arg1;
+ (const struct WCTProperty *)updateTime;
+ (void)__wcdb_WAConfigInfo_synthesize_4:(struct WCTBinding *)arg1;
+ (const struct WCTProperty *)configData;
+ (void)__wcdb_WAConfigInfo_synthesize_3:(struct WCTBinding *)arg1;
+ (const struct WCTProperty *)version;
+ (void)__wcdb_WAConfigInfo_synthesize_2:(struct WCTBinding *)arg1;
+ (const struct WCTProperty *)type;
+ (void)__wcdb_WAConfigInfo_synthesize_1:(struct WCTBinding *)arg1;
+ (const struct WCTProperty *)appID;
+ (void)__wcdb_WAConfigInfo_synthesize_0:(struct WCTBinding *)arg1;
+ (CDUnknownBlockType)columnNamed;
+ (const struct All *)allResults;
+ (const struct WCTPropertyList *)allProperties;
+ (const struct WCTBinding *)objectRelationalMapping;
@property(nonatomic) unsigned int updateTime; // @synthesize updateTime;
@property(retain, nonatomic) NSData *configData; // @synthesize configData;
@property(nonatomic) unsigned int version; // @synthesize version;
@property(nonatomic) unsigned int type; // @synthesize type;
@property(retain, nonatomic) NSString *appID; // @synthesize appID;
- (void).cxx_destruct;

// Remaining properties
@property(nonatomic) _Bool isAutoIncrement;
@property(nonatomic) long long lastInsertedRowID;

@end

